...
val commad: Command[Short *: String *: EmptyTuple]=sql"INSERT INTO foo VALUES ($int2, $varchar)".command // Command[(Short, String)]
// Query-> SQL, parameter encoder, and row decoder for a statement that returns rows

val query: Query[Short,String *: Short *: EmptyTuple]=sql"SELECT name, age FROM person WHERE age > $int2".query(varchar *: int2) // Query[Short, (String, Short)]
