...
//Command[A] A represening the number of parameters
    val command: Command[Short *: String *: EmptyTuple]=sql"INSERT INTO foo VALUES ($int2, $varchar)".command
...
session.use{_.prepare(c).flatMap { pc =>
  pc.execute("xyzzy") *>
  pc.execute("fnord") *>
  pc.execute("blech")
}
...
Stream.eval(session.use(_.prepare(c))).flatMap { pc =>
